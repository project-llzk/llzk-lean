...
require_not_contains docs/harness/CURRENT.md "starts moving the corpus evidence" "CURRENT no longer says clean-pin corpus migration is only starting"
...
if grep -q "differential/run-differential.sh" "${ROOT}/docs/harness/SOURCES.md" &&
   grep -q "differential/corpus/" "${ROOT}/docs/harness/SOURCES.md"; then
  ok "SOURCES records Strategy A differential wrapper and corpus"
else
  fail "SOURCES does not record Strategy A differential wrapper and corpus"
fi
...
for mnemonic in sub mul neg; do
  require_op_row_contains "$mnemonic" "Gap in Strategy A/E coverage" "FELT_OP_GAPS marks ${mnemonic} Strategy A/E coverage gap"
done

for mnemonic in pow div uintdiv sintdiv umod smod inv bit_and bit_or bit_xor bit_not shl shr; do
  require_op_row_contains "$mnemonic" 'Missing from consumed VeIR `Data.Felt` and `InterpModel`' "FELT_OP_GAPS marks ${mnemonic} consumed VeIR semantic model missing"
...
for evidence in \
  ...
  reviews/PHASE-03/evidence/verify-pins.txt \
  reviews/PHASE-03/evidence/doctor-workspace.txt \
...
for evidence in \
  ...
  reviews/PHASE-04/evidence/verify-pins.txt \
  reviews/PHASE-04/evidence/doctor-workspace.txt \
...
for evidence in \
  ...
  reviews/PHASE-05/evidence/verify-pins.txt \
  reviews/PHASE-05/evidence/doctor-workspace.txt \
  reviews/PHASE-05/evidence/validate-skills.txt \
  reviews/PHASE-05/evidence/lake-build.txt \
  reviews/PHASE-05/evidence/skip-guard.txt \
...
for evidence in \
  ...
  reviews/PHASE-06/evidence/verify-pins.txt \
  reviews/PHASE-06/evidence/doctor-workspace.txt \
...
for evidence in \
  ...
  reviews/PHASE-07/evidence/verify-pins.txt \
  reviews/PHASE-07/evidence/doctor-workspace.txt \
  reviews/PHASE-07/evidence/validate-skills.txt \
  reviews/PHASE-07/evidence/lake-build.txt \
  reviews/PHASE-07/evidence/cert-smoke.txt \
...
require_contains reviews/PHASE-03/evidence/verify-pins.txt "pin verification summary: 0 fail" "pin evidence reports no failures"
require_contains reviews/PHASE-03/evidence/doctor-workspace.txt "doctor summary: 0 fail" "strict doctor evidence reports no failures"
...
require_contains reviews/PHASE-03/evidence/lake-build.txt "catalog covers 2 of them; 13 uncovered" "lake build evidence records expected certificate coverage gap"
require_contains reviews/PHASE-03/evidence/adversarial-review.txt "PASS: no missing-op semantic definitions found in consumed VeIR Felt semantics" "adversarial evidence confirms missing-op semantics were not added"
require_contains reviews/PHASE-03/evidence/adversarial-review.txt "PASS: no modified, staged, or untracked files under Lean cert/catalog/corpus implementation paths" "adversarial evidence confirms no Strategy E/A implementation changes"
...
require_contains reviews/PHASE-04/evidence/verify-pins.txt "pin verification summary: 0 fail" "Phase 4 pin evidence reports no failures"
require_contains reviews/PHASE-04/evidence/doctor-workspace.txt "doctor summary: 0 fail" "Phase 4 strict doctor evidence reports no failures"
...
require_contains reviews/PHASE-04/evidence/adversarial-review.txt "PASS: canonicalization-aware differential wrapper and corpus are dispositioned under reviews/PHASE-04" "Phase 4 adversarial evidence dispositions differential wrapper and corpus edits"
...
require_contains reviews/PHASE-04/evidence/adversarial-review-fresh.txt "PASS: llzk-lean canonical run without VEIR_DIFF now exits early with an actionable clean-pin/override message." "Phase 4 fresh adversarial evidence records clean-pin guard"
...
require_contains reviews/PHASE-05/evidence/verify-pins.txt "pin verification summary: 0 fail" "Phase 5 pin evidence reports no failures"
require_contains reviews/PHASE-05/evidence/doctor-workspace.txt "doctor summary: 0 fail" "Phase 5 strict doctor evidence reports no failures"
...
require_contains reviews/PHASE-05/evidence/skip-guard.txt "ERROR: 1 input(s) skipped because required tools were unavailable; this is not acceptance evidence." "Phase 5 skip guard rejects missing-tool evidence"
...
require_contains reviews/PHASE-05/evidence/adversarial-review.txt "PASS: expected-divergence polarity is marker-specific" "Phase 5 adversarial evidence records exact polarity guard"
...
require_contains reviews/PHASE-06/evidence/verify-pins.txt "pin verification summary: 0 fail" "Phase 6 pin evidence reports no failures"
require_contains reviews/PHASE-06/evidence/doctor-workspace.txt "doctor summary: 0 fail" "Phase 6 strict doctor evidence reports no failures"
...
require_contains reviews/PHASE-06/evidence/adversarial-review.txt "PASS: expected-divergence polarity remains exact and marker-driven." "Phase 6 adversarial evidence records exact polarity baseline"
...
require_contains reviews/PHASE-07/evidence/verify-pins.txt "pin verification summary: 0 fail" "Phase 7 pin evidence reports no failures"
require_contains reviews/PHASE-07/evidence/doctor-workspace.txt "doctor summary: 0 fail" "Phase 7 strict doctor evidence reports no failures"
...
require_contains reviews/PHASE-07/evidence/cert-smoke.txt "CERT-SMOKE: schema validation passed" "Phase 7 certificate smoke evidence reports success"
require_contains reviews/PHASE-07/evidence/cert-smoke.txt "CERT-SMOKE: theorem metadata present for 2/2 certs" "Phase 7 certificate smoke evidence validates theorem metadata"
...
require_contains reviews/PHASE-07/evidence/adversarial-review.txt "PASS: expected-divergence polarity remains exact and marker-driven." "Phase 7 adversarial evidence records exact polarity baseline"
...
require_contains reviews/PHASE-07/evidence/adversarial-review.txt "PASS: certificate smoke passes after the constant_fold_add parity update." "Phase 7 adversarial evidence records certificate-smoke closeout"
...
require_contains reviews/PHASE-08/evidence/differential-clean-pin-canonicalize.txt '$ env -u VEIR_DIFF -u VEIR_OPT LLZK_OPT=/nix/store/awcw2wiypa02sl5vx4xm06qwji68xz3h-llzk-debug-2.0.0/bin/llzk-opt ./differential/run-differential.sh --canonicalize differential/corpus' "Phase 8 differential evidence records exact clean-pin command"
require_contains reviews/PHASE-08/evidence/differential-clean-pin-canonicalize.txt "CLEAN-VEIR-OPT: lake build veir-opt succeeded for /home/alh/LLZK/llzk-lean/.lake/packages/VeIR" "Phase 8 differential evidence records pinned veir-opt refresh"
...
require_contains reviews/PHASE-08/evidence/adversarial-review.txt "PASS: clean-pin differential refreshes pinned dependency veir-opt before comparison." "Phase 8 adversarial evidence records veir-opt refresh"
require_contains reviews/PHASE-08/evidence/adversarial-review.txt "PASS: clean-pin differential rejects hidden VEIR_OPT overrides." "Phase 8 adversarial evidence records hidden override guard"
require_contains reviews/PHASE-08/evidence/adversarial-review.txt "PASS: Phase 8 differential evidence records env -u VEIR_DIFF -u VEIR_OPT and the accepted LLZK_OPT path." "Phase 8 adversarial evidence records exact command evidence"
...
require_not_contains differential/README.md "corpus is still a reviewed seed" "differential README no longer calls the Phase 5 corpus a seed"
require_not_contains differential/README.md "Current seed bar" "differential README no longer carries the stale seed bar"
...
if grep -q "Phase 8: Strategy A Field Preconditions" "${ROOT}/docs/phases/PHASE-08-strategy-a-field-preconditions.md" &&
   ...
   grep -q "21 pass (incl. expected-diverge), 0 fail" "${ROOT}/docs/phases/PHASE-08-strategy-a-field-preconditions.md"; then
